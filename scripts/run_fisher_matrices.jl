...
@enum ExperimentType fisher = 1 wishart = 2 fisher_factor = 3 wishart_factor = 4 wigner = 5 uniform = 6
...
function experiment_fixed_spectrum(exp_type::ExperimentType)
        @match exp_type begin
                $fisher => [5.0 + 10 * 1.5^(-i + 1) for i in 1:20]
                $fisher_factor => [2.7 + 10 * 1.5^(-i + 1) for i in 1:20]
                $wishart => [22, 20]
                $wishart_factor => [1 + 10 * 1.5^(-i + 1) for i in 1:20]
                $wigner => [1 + 10 * 1.4^(-i + 1) for i in 1:20]
                $uniform => [3.0 + 10 * 1.4^(-i + 1) for i in 1:20]
            _ => error("Unknown experiment type")
        end
end
...
function main()
        ...
        proportions = [0.5, 1]
        for exp_type in instances(ExperimentType)
                if exp_type != uniform
                        continue
                end
