...
function save_results(results::FDRControlSubspaceSelection.FDRResult, alpha::Float64, output_folder::String)
    data = Dict(
    ...
    plot_spacings(results, joinpath(output_folder, "spacings.pdf"), 3 * results.rank_estimate)
    plot_fdr(results, joinpath(output_folder, "fdr.pdf"), 3 * results.rank_estimate, threshold=alpha)
    plot_eigenvalues(results, joinpath(output_folder, "eigenvalues.pdf"), 3 * results.rank_estimate)
end
